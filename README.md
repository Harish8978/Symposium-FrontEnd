...
```bash
# Install dependencies (uses .npmrc for legacy-peer-deps)
npm install

# Start web (browser) - DEFAULT - No Android SDK needed!
npm start

# Or use explicit commands:
npm run start-web      # Web browser
npm run start-dev      # Dev server with QR code (for Expo Go)
npm run start-android  # Android emulator (requires Android SDK)
npm run start-ios      # iOS simulator (Mac only)
```

**Note:** `npm start` now defaults to web mode to avoid Android SDK errors. See `ANDROID_SETUP.md` if you need Android emulator.
